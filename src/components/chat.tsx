    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed bottom-0 right-0 sm:bottom-6 sm:right-6 h-full w-full sm:h-[70vh] sm:max-h-[700px] sm:w-[440px] z-50 flex flex-col"
        >
          <div className="bg-card text-card-foreground rounded-t-lg sm:rounded-lg shadow-2xl border border-border/50 flex flex-col h-full">
            {/* Header */}
            <header className="flex items-center justify-between p-4 border-b border-border/50">
                <div className="flex items-center gap-3">
                    {logoUrl && <Image src={logoUrl} alt="Logo" width={32} height={32} className="rounded-full"/>}
                    <div>
